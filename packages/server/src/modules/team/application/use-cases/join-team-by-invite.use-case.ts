...
export class JoinTeamByInviteUseCase {
    ...
    async execute(code: string, userId: string) {
        ...
        if (!invite || !team.isInviteActive(code, this.clock.now())) throw new BadRequestException('만료되었거나 비활성화된 초대 링크입니다.');
        if (team.isMember(userId)) return team;

        const nextMembers: TeamMember[] = [
            ...team.members,
            { userId, role: invite.defaultRole, joinedAt: this.clock.now(), canManageInvites: false },
        ];
