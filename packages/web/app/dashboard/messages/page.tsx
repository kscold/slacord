...
export default function MessagesPage() {
    ...
    return (
        <DashboardShell title="메시지 검색" description="접근 가능한 워크스페이스의 최근 메시지를 한 번에 보고, 필요한 대화로 바로 이동할 수 있도록 구성했습니다." currentUserName={search.currentUserName}>
            <div className="space-y-6">
                ...
                {!search.booting ? <SearchResultList indexing={search.indexing} pinnedResults={search.pinnedResults} query={search.query} recentResults={search.recentResults} results={search.results} teamCount={search.teamCount} /> : null}
            </div>
        </DashboardShell>
    );
}
