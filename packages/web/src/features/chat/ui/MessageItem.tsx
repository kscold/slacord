...
export function MessageItem({ message, currentUserId, onReact, onDelete }: Props) {
        ...
        <div className={`group flex gap-3 px-4 py-1.5 hover:bg-bg-hover/30 transition-colors ${isOwn ? 'flex-row-reverse' : ''}`}>
            ...
            <div className={`max-w-[70%] ${isOwn ? 'items-end' : 'items-start'} flex flex-col`}>
                <div className="flex items-baseline gap-2 mb-0.5">
                    <span className="text-xs font-semibold text-text-secondary">{message.authorId.slice(-6)}</span>
