...
export class RelayService implements OnModuleInit {
  ...
  async onModuleInit() {
    ...
    if (channelIds) {
      ...
      this.logger.log(
        `[onModuleInit] 백업 대상 채널: ${this.targetChannels.join(', ')}`,
      );
    } else {
      this.logger.warn(
        '[onModuleInit] SLACK_TARGET_CHANNELS 미설정. 모든 채널 백업.',
      );
    }
  ...
  private async handleSlackMessage(message: any): Promise<void> {
    try {
      ...
      // 채널 필터링 (설정된 채널만 백업)
      if (
        this.targetChannels.length > 0 &&
        !this.targetChannels.includes(channel)
      ) {
        this.logger.debug(
          `[handleSlackMessage] 채널 ${channel}은 백업 대상이 아닙니다.`,
        );
        return;
      }
      ...
      this.logger.log(
        `[handleSlackMessage] 메시지 백업: #${channelName} - ${username}`,
      );
      ...
      if (files && files.length > 0) {
        ...
        for (const file of files) {
          await this.discordService.sendMessageWithFile(
            formattedMessage,
            file.url_private,
            username,
          );
      ...
      this.logger.error(
        `[handleSlackMessage] 메시지 처리 실패: ${error.message}`,
        error.stack,
      );
    }
  }
