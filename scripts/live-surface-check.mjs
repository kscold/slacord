...
const apiBaseUrl = normalizeBaseUrl(process.env.SLACORD_LIVE_API_URL || 'http://127.0.0.1:8084/api');
const socketBaseUrl = normalizeBaseUrl(process.env.SLACORD_LIVE_SOCKET_URL || apiBaseUrl.replace(/\/api$/, ''));
const reportDir = path.resolve(process.env.SLACORD_LIVE_REPORT_DIR || './artifacts/live-surface-check');
...
async function api(pathName, options = {}) {
    ...
    if (options.expectOk !== false && !response.ok) {
        throw new Error(`HTTP ${response.status} ${pathName}: ${summarize(payload)}`);
    }
...
async function expectFailure(pathName, options = {}, matcher) {
    ...
    if (matcher && !matcher(response)) {
        throw new Error(`실패 응답이 기대와 다릅니다: ${summarize(response.payload)}`);
    }
    return response;
}
...
function waitForEvent(socket, eventName, predicate = () => true, timeoutMs = 5000) {
    return new Promise((resolve, reject) => {
        const timer = setTimeout(() => {
            cleanup();
            reject(new Error(`${eventName} 이벤트를 ${timeoutMs}ms 안에 받지 못했습니다.`));
        }, timeoutMs);
...
function expectNoEvent(socket, eventName, predicate = () => true, timeoutMs = 1200) {
    return new Promise((resolve, reject) => {
        const timer = setTimeout(() => {
            cleanup();
            resolve(null);
        ...
        const onEvent = (payload) => {
            try {
                if (!predicate(payload)) return;
                cleanup();
                reject(new Error(`${eventName} 이벤트가 오면 안 되는데 수신했습니다: ${summarize(payload)}`));
            } catch (error) {
                cleanup();
                reject(error);
            }
...
async function emitWithAck(socket, eventName, payload) {
    if (typeof socket.timeout === 'function' && typeof socket.timeout(5000).emitWithAck === 'function') {
...
async function joinChannel(socket, channelId) {
    const joined = waitForEvent(socket, 'joined_channel', (payload) => payload?.channelId === channelId);
...
async function main() {
    ...
    assert(health.payload?.status === 'ok' || health.payload?.success !== false, '대상 API가 정상 응답하지 않습니다.');

    await check('setup', '사용자 5명 생성', async () => {
    ...
    await check('team', '초대 링크 미리보기', async () => {
        const response = await api(`/team/invite/${oneTimeInvite}`, { expectOk: true });
        assert(response.payload?.data?.code === oneTimeInvite, '초대 미리보기 코드가 다릅니다.');
    ...
    await check('team', '오너가 actor를 admin으로 승격', async () => {
        const response = await api(`/team/${context.teamId}/member/${context.users.actor.userId}/access`, {
            method: 'PATCH',
            token: context.users.owner.token,
            body: { role: 'admin' },
        });
        assert(response.payload?.data?.role === 'admin', 'admin 승격이 반영되지 않았습니다.');
        return 'actor -> admin';
    });
    ...
    await check('team', '초대 링크 삭제 후 미리보기 차단', async () => {
        ...
        await expectFailure(`/team/invite/${deleteCode}`, {}, (response) => response.status >= 400);
        return deleteCode;
    });
    ...
    await check('team', '팀 멤버 목록 조회', async () => {
        ...
        const memberIds = new Set((response.payload?.data || []).map((member) => member.userId));
        assert(memberIds.has(context.users.owner.userId), 'owner가 멤버 목록에 없습니다.');
        assert(memberIds.has(context.users.actor.userId), 'actor가 멤버 목록에 없습니다.');
        assert(memberIds.has(context.users.allowedMember.userId), 'allowedMember가 멤버 목록에 없습니다.');
        assert(memberIds.has(context.users.deniedMember.userId), 'deniedMember가 멤버 목록에 없습니다.');
    ...
    await check('channel', '공개 채널 생성', async () => {
        const response = await api(`/team/${context.teamId}/channel`, {
            method: 'POST',
            token: context.users.owner.token,
            body: { name: `general-${Date.now().toString().slice(-4)}`, type: 'public' },
        });
    ...
    await check('channel', 'DM 생성 및 중복 생성 시 재사용', async () => {
        ...
        assert(first.payload?.data?.id === second.payload?.data?.id, '같은 DM이 재사용되지 않았습니다.');
    ...
    await check('channel', '소그룹 채널 생성', async () => {
        const response = await api(`/team/${context.teamId}/channel`, {
            ...
            body: {
                name: 'group-room',
                type: 'group',
                memberIds: [context.users.actor.userId, context.users.allowedMember.userId],
            },
        });
    ...
    await check('channel', '채널 가시성 필터링', async () => {
        const actorChannels = await api(`/team/${context.teamId}/channel`, { token: context.users.actor.token });
        const deniedChannels = await api(`/team/${context.teamId}/channel`, { token: context.users.deniedMember.token });
        const actorIds = new Set((actorChannels.payload?.data || []).map((channel) => channel.id));
        const deniedIds = new Set((deniedChannels.payload?.data || []).map((channel) => channel.id));
        assert(actorIds.has(context.channels.private), 'private channel이 actor에게 보여야 합니다.');
        assert(actorIds.has(context.channels.dm), 'dm channel이 actor에게 보여야 합니다.');
        assert(deniedIds.has(context.channels.public), 'public channel이 일반 멤버에게 보여야 합니다.');
        assert(!deniedIds.has(context.channels.private), 'private channel이 비멤버에게 보이면 안 됩니다.');
        assert(!deniedIds.has(context.channels.dm), 'dm channel이 비멤버에게 보이면 안 됩니다.');
    ...
    await check('channel', 'voice 채널 가시성', async () => {
        const ownerChannels = await api(`/team/${context.teamId}/channel`, { token: context.users.owner.token });
        const deniedChannels = await api(`/team/${context.teamId}/channel`, { token: context.users.deniedMember.token });
        const ownerIds = new Set((ownerChannels.payload?.data || []).map((channel) => channel.id));
        const deniedIds = new Set((deniedChannels.payload?.data || []).map((channel) => channel.id));
        assert(ownerIds.has(context.channels.voice), 'voice channel이 owner에게 보여야 합니다.');
        assert(deniedIds.has(context.channels.voice), 'voice channel이 일반 멤버에게 보여야 합니다.');
    ...
    await check('notification', '알림 소켓 연결 및 팀 룸 참여', async () => {
        sockets.ownerNotification = await connectSocket('notification', context.users.owner.token, 'owner notification');
        const joined = waitForEvent(
            sockets.ownerNotification,
            'joined_team_notifications',
            (payload) => payload?.teamId === context.teamId,
        );
        sockets.ownerNotification.emit('join_team_notifications', { teamId: context.teamId });
        await joined;
        return 'notification room joined';
    });

    await check('message', '채팅 소켓 연결 및 공개 채널 입장', async () => {
        sockets.ownerChat = await connectSocket('chat', context.users.owner.token, 'owner chat');
        sockets.actorChat = await connectSocket('chat', context.users.actor.token, 'actor chat');
        sockets.allowedChat = await connectSocket('chat', context.users.allowedMember.token, 'allowed chat');
        sockets.deniedChat = await connectSocket('chat', context.users.deniedMember.token, 'denied chat');
        sockets.outsiderChat = await connectSocket('chat', context.users.outsider.token, 'outsider chat');
        ...
        return 'public room joined by 4 sockets';
    });

    await check('security', 'outsider는 공개 채널 소켓 join을 할 수 없어야 함', async () => {
        ...
        sockets.outsiderChat.emit('join_channel', { channelId: context.channels.public });
        await denied;
        await joined;
    ...
    await check('message', '공개 채널 메시지 전송 및 실시간 수신', async () => {
        const incoming = waitForEvent(
            sockets.actorChat,
            'new_message',
            (payload) => payload?.channelId === context.channels.public && payload?.content?.includes(`@${context.users.actor.username}`),
        );
    ...
    await check('message', '메시지 REST 조회와 mention 파싱', async () => {
        ...
        const message = (response.payload?.data || []).find((item) => item.id === context.messageIds.publicMessage);
        assert(message, '방금 보낸 메시지를 조회하지 못했습니다.');
        assert(Array.isArray(message.mentions) && message.mentions.includes(context.users.actor.username), 'mention 파싱이 되지 않았습니다.');
    ...
    await check('message', '반응 추가/제거', async () => {
        const addedEvent = waitForEvent(
            sockets.ownerChat,
            'reaction_updated',
            (payload) => payload?.id === context.messageIds.publicMessage && payload?.reactions?.[0]?.emoji === '👍',
        );
        ...
        assert(added.reactions[0].userIds.includes(context.users.actor.userId), '반응 userIds가 비어 있습니다.');

        const removedEvent = waitForEvent(
            sockets.ownerChat,
            'reaction_updated',
            (payload) => payload?.id === context.messageIds.publicMessage && (payload?.reactions?.length || 0) === 0,
        );
        await emitWithAck(sockets.actorChat, 'add_reaction', {
            channelId: context.channels.public,
            messageId: context.messageIds.publicMessage,
    ...
    await check('message', '작성자만 메시지 편집 가능', async () => {
        await expectFailure(`/channel/${context.channels.public}/message/${context.messageIds.publicMessage}`, {
            method: 'PATCH',
            token: context.users.deniedMember.token,
            body: { content: 'hijacked edit' },
        });
        await api(`/channel/${context.channels.public}/message/${context.messageIds.publicMessage}`, {
            method: 'PATCH',
            token: context.users.owner.token,
        ...
        const edited = (response.payload?.data || []).find((item) => item.id === context.messageIds.publicMessage);
        assert(edited?.isEdited === true, 'isEdited가 true가 아닙니다.');
        assert(edited?.content === 'edited by owner', '편집 내용이 반영되지 않았습니다.');
        return 'edit ok';
    });
    ...
    await check('security', 'outsider는 비공개 채널 소켓 join을 할 수 없어야 함', async () => {
        const joined = expectNoEvent(
            sockets.outsiderChat,
            'joined_channel',
        ...
        sockets.outsiderChat.emit('join_channel', { channelId: context.channels.private });
        await denied;
        await joined;
        return 'outsider private join denied';
    });

    await check('security', '비멤버는 비공개 채널 메시지를 조회할 수 없어야 함', async () => {
    ...
    await check('security', '비멤버는 다른 팀 메시지를 핀할 수 없어야 함', async () => {
        await expectFailure(`/channel/${context.channels.public}/message/${context.messageIds.publicMessage}/pin`, {
            method: 'PATCH',
            token: context.users.outsider.token,
            body: { isPinned: true },
        });
    ...
    await check('message', '메시지 핀과 핀 목록 조회', async () => {
        await api(`/channel/${context.channels.public}/message/${context.messageIds.publicMessage}/pin`, {
            method: 'PATCH',
            token: context.users.owner.token,
            body: { isPinned: true },
        });
        const pinned = await api(`/channel/${context.channels.public}/message/pinned`, {
            token: context.users.owner.token,
        });
        const found = (pinned.payload?.data || []).find((item) => item.id === context.messageIds.publicMessage);
    ...
    await check('notification', '스레드 답글 알림 생성', async () => {
        const incoming = waitForEvent(
            sockets.ownerNotification,
            'notification:new',
            (payload) => payload?.type === 'thread_reply' && payload?.channelId === context.channels.public,
            7000,
        );
        ...
        assert(notification.resourceType === 'message', 'thread reply 알림 resourceType이 다릅니다.');
        return notification.id;
    });

    await check('message', '스레드 답글 조회', async () => {
        const response = await api(`/channel/${context.channels.public}/message/${context.messageIds.publicMessage}/thread`, {
            token: context.users.owner.token,
        });
        const found = (response.payload?.data || []).find((item) => item.id === context.messageIds.threadReply);
    ...
    await check('message', '첨부 업로드 후 파일 메시지 전송', async () => {
        ...
        formData.append('file', createTextFile('note.txt', 'attachment smoke'));

        const uploaded = await api(`/channel/${context.channels.public}/message/attachment`, {
            method: 'POST',
            token: context.users.owner.token,
            formData,
        });
    ...
    await check('message', 'admin이 다른 사람 메시지를 삭제할 수 있어야 함', async () => {
        await api(`/channel/${context.channels.public}/message/${context.messageIds.publicMessage}`, {
            method: 'DELETE',
            token: context.users.actor.token,
        });
    ...
    await check('announcement', '공지 생성과 핀 우선 정렬', async () => {
        ...
        await api(`/team/${context.teamId}/announcement/${second.payload?.data?.id}/pin`, {
            method: 'PATCH',
            token: context.users.owner.token,
            body: { isPinned: true },
        });
        ...
        assert(list.payload?.data?.[0]?.id === second.payload?.data?.id, '핀된 공지가 첫 번째에 오지 않습니다.');
    ...
    await check('issue', '이슈 필터와 상태 업데이트', async () => {
        ...
        const byQuery = await api(`/team/${context.teamId}/issue?q=${encodeURIComponent('자동 테스트 이슈')}`, {
            token: context.users.owner.token,
        });
        const byStatus = await api(`/team/${context.teamId}/issue?status=in_progress`, {
            token: context.users.owner.token,
        });
        const byAssignee = await api(`/team/${context.teamId}/issue?assigneeId=${context.users.owner.userId}`, {
            token: context.users.owner.token,
        });
        assert((byQuery.payload?.data || []).some((issue) => issue.id === context.issueIds.primary), 'q 필터가 동작하지 않습니다.');
        assert((byStatus.payload?.data || []).some((issue) => issue.id === context.issueIds.primary), 'status 필터가 동작하지 않습니다.');
        assert((byAssignee.payload?.data || []).some((issue) => issue.id === context.issueIds.primary), 'assignee 필터가 동작하지 않습니다.');
        return 'filters ok';
    });
    ...
    await check('issue', '이슈 삭제', async () => {
        ...
        const list = await api(`/team/${context.teamId}/issue?q=${encodeURIComponent('삭제용 이슈')}`, {
            token: context.users.owner.token,
        });
        assert(!(list.payload?.data || []).some((issue) => issue.id === tempId), '삭제한 이슈가 남아 있습니다.');
        return tempId;
    });
    ...
    await check('notification', '알림 목록/개수 조회와 읽음 처리', async () => {
        ...
        const count = await api(`/team/${context.teamId}/notification/unread-count`, {
            token: context.users.owner.token,
        });
        const notifications = list.payload?.data || [];
        assert(notifications.length >= 2, '알림이 2개 이상 쌓이지 않았습니다.');
        assert(count.payload?.data?.count >= 2, '읽지 않은 알림 수가 기대보다 적습니다.');
        ...
        const after = await api(`/team/${context.teamId}/notification/unread-count`, {
            token: context.users.owner.token,
        });
        assert(after.payload?.data?.count === 0, 'read-all 이후 unread count가 0이 아닙니다.');
        return `${notifications.length} notifications`;
    });

    await check('presence', '프레즌스 소켓 연결과 상태 변경', async () => {
        sockets.ownerPresence = await connectSocket('presence', context.users.owner.token, 'owner presence');
        sockets.actorPresence = await connectSocket('presence', context.users.actor.token, 'actor presence');

        const ownerOnline = waitForEvent(
            sockets.ownerPresence,
            'presence_update',
            (payload) => payload?.userId === context.users.actor.userId && payload?.status === 'online',
            7000,
        );
        ...
        const awayUpdate = waitForEvent(
            sockets.ownerPresence,
            'presence_update',
            (payload) => payload?.userId === context.users.actor.userId && payload?.status === 'away',
            7000,
        );
        sockets.actorPresence.emit('set_status', { status: 'away' });
        await awayUpdate;
        ...
        const actorPresence = (rest.payload?.data || []).find((item) => item.userId === context.users.actor.userId);
        assert(actorPresence?.status === 'away', 'REST presence가 away를 반환하지 않습니다.');
        return actorPresence.status;
    });

    await check('security', '비멤버는 프레즌스 조회를 할 수 없어야 함', async () => {
    ...
    await check('document', '문서 생성과 트리 조회', async () => {
        ...
        assert(context.documentIds.root && context.documentIds.child, '문서 생성 id가 없습니다.');
    ...
    await check('document', '문서 수정과 버전 복원', async () => {
        ...
        const versions = await api(`/team/${context.teamId}/document/${context.documentIds.root}/version`, {
            token: context.users.owner.token,
        });
        const versionId = versions.payload?.data?.[0]?.id;
        assert(versionId, '문서 버전이 생성되지 않았습니다.');
        await api(`/team/${context.teamId}/document/${context.documentIds.root}/version/${versionId}/restore`, {
            method: 'POST',
            token: context.users.owner.token,
        });
        const restored = await api(`/team/${context.teamId}/document/${context.documentIds.root}`, {
            token: context.users.owner.token,
        });
        assert(restored.payload?.data?.title === 'Root Doc', '버전 복원 후 title이 원복되지 않았습니다.');
        return versionId;
    });
    ...
    await check('document', '제한 문서 RBAC', async () => {
        ...
        const allowedGet = await api(`/team/${context.teamId}/document/${context.documentIds.root}`, {
            token: context.users.allowedMember.token,
        });
        assert(allowedGet.payload?.data?.id === context.documentIds.root, '허용 멤버가 문서를 보지 못합니다.');

        await expectFailure(`/team/${context.teamId}/document/${context.documentIds.root}`, {
            token: context.users.deniedMember.token,
        });
    ...
    await check('document', '문서 이미지/파일 업로드', async () => {
        ...
        const file = await api(`/team/${context.teamId}/document/${context.documentIds.root}/file`, {
            method: 'POST',
            token: context.users.owner.token,
            formData: fileForm,
        });
    ...
    await check('document', '문서 아카이브와 복원', async () => {
        await api(`/team/${context.teamId}/document/${context.documentIds.child}/archive`, {
            method: 'POST',
            token: context.users.owner.token,
        });
        const archived = await api(`/team/${context.teamId}/document/archived/list`, {
            token: context.users.owner.token,
        });
        assert((archived.payload?.data || []).some((doc) => doc.id === context.documentIds.child), '아카이브 문서 목록에 child가 없습니다.');
        await api(`/team/${context.teamId}/document/${context.documentIds.child}/restore`, {
            method: 'POST',
            token: context.users.owner.token,
        });
        const tree = await api(`/team/${context.teamId}/document`, {
        ...
        assert((tree.payload?.data || []).some((doc) => doc.id === context.documentIds.child), '복원 후 child doc이 트리에 없습니다.');
    ...
    await check('integration', '일반 멤버는 Discord import를 실행할 수 없어야 함', async () => {
    ...
    await check('integration', 'Discord import 엔드포인트 실행 경로 확인', async () => {
        await expectFailure(`/team/${context.teamId}/discord/import`, {
            method: 'POST',
            token: context.users.owner.token,
            body: {
    ...
    await check('integration', '비멤버는 GitHub 설정을 바꾸면 안 됨', async () => {
    ...
    await check('integration', 'GitHub 설정과 Webhook 수신', async () => {
        ...
        const invalid = await api('/github/webhook', {
            ...
            body: {
                ...
                pull_request: { number: 7, title: 'Webhook test', html_url: `https://github.com/${repoFullName}/pull/7` },
            },
            expectOk: true,
        });
        assert(invalid.payload?.success === false, '잘못된 서명이 거절되지 않았습니다.');
        ...
        assert(validPayload?.success === true, '유효한 GitHub webhook이 처리되지 않았습니다.');

        const messages = await api(`/channel/${context.channels.public}/message`, {
            token: context.users.owner.token,
        });
        const githubMessage = (messages.payload?.data || []).find((message) => typeof message.content === 'string' && message.content.includes('[PR #7] Webhook test'));
        assert(githubMessage, 'GitHub webhook 메시지가 채널에 저장되지 않았습니다.');
    ...
    await check('integration', 'Confluence 가져오기', async () => {
        ...
        if (!stub.port) {
            await skip('integration', 'Confluence 가져오기', 'stub port를 잡지 못했습니다.');
            return 'skipped';
        }

        try {
            const response = await api(`/team/${context.teamId}/document/import/confluence`, {
                method: 'POST',
                token: context.users.owner.token,
                body: {
            ...
            assert(response.payload?.data?.importedCount === 2, 'Confluence importedCount가 2가 아닙니다.');

            const docs = await api(`/team/${context.teamId}/document`, {
                token: context.users.owner.token,
            });
            const titles = new Set((docs.payload?.data || []).map((doc) => doc.title));
            assert(titles.has('Confluence Root'), 'Confluence Root 문서가 없습니다.');
            assert(titles.has('Confluence Child'), 'Confluence Child 문서가 없습니다.');
            return '2 imported';
        } finally {
    ...
    await check('presence', '프레즌스 오프라인 전환', async () => {
        const offlineEvent = waitForEvent(
            sockets.ownerPresence,
            'presence_update',
            (payload) => payload?.userId === context.users.actor.userId && payload?.status === 'offline',
            7000,
        );
        ...
        const actorPresence = (rest.payload?.data || []).find((item) => item.userId === context.users.actor.userId);
        assert(actorPresence?.status === 'offline', 'REST presence가 offline을 반환하지 않습니다.');
        return 'offline ok';
    });
    ...
    await check('huddle', '허들 참여자 목록과 미디어 토글', async () => {
        const ownerParticipants = waitForEvent(
            sockets.ownerChat,
            'huddle:participants',
            (payload) => payload?.channelId === context.channels.public && Array.isArray(payload.participants) && payload.participants.length >= 1,
            7000,
        );
        sockets.ownerChat.emit('huddle:join', { channelId: context.channels.public });
        await ownerParticipants;

        const actorParticipants = waitForEvent(
            sockets.ownerChat,
            'huddle:participants',
            (payload) => payload?.channelId === context.channels.public && payload.participants.some((item) => item.userId === context.users.actor.userId),
            7000,
        );
        sockets.actorChat.emit('huddle:join', { channelId: context.channels.public });
        await actorParticipants;

        const toggleEvent = waitForEvent(
            sockets.ownerChat,
            'huddle:participants',
            (payload) => payload?.channelId === context.channels.public && payload.participants.some((item) => item.userId === context.users.actor.userId && item.video === true),
            7000,
        );
    ...
    await check('huddle', '허들 offer는 targetUserId에게만 가야 함', async () => {
        const participantEvent = waitForEvent(
            sockets.ownerChat,
            'huddle:participants',
            (payload) => payload?.channelId === context.channels.public && payload.participants.some((item) => item.userId === context.users.allowedMember.userId),
            7000,
        );
        sockets.allowedChat.emit('huddle:join', { channelId: context.channels.public });
...
try {
    ...
    console.log(`summary: passed=${summary.passed} failed=${summary.failed} skipped=${summary.skipped}`);
    if (summary.failed > 0) {
        process.exitCode = 1;
    }
}
